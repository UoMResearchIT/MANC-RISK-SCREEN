# Code generated by parse_ui_table.R, Wed May 24 13:00:16 2023
...
fluidPage(
  ...
  fluidRow(
   ...
   # This can be fixed as there are some hard checks in the model
    ...
    column(width=NCOL, sliderInput("discount_health", "Health discount rate", step = 0.005, value = 0.035, min = 0, max = 0.06) ),
    column(width=NCOL, sliderInput("discount_cost", "Cost discount rate", step = 0.005, value = 0.035, min = 0, max = 0.06) ),
    column(width=NCOL, sliderInput("screen_startage", "Screen start age", value = 50, min = 18, max = 60) ),
    column(width=NCOL, sliderInput("screen_endage", "Screen end age", value = 70, min = 18, max = 80) )
  ),
  h2("Uptake"),
  fluidRow(
  ...
  h2("Cancer and all cause mortality"),
  fluidRow(
...
cols = list(names = T, editableNames = F),
class = "numeric") ),
    column(width=NCOL, numericInput("screen_detection_m", "Mean doubling rate for tumours at screen detection", value = 6.12, min = 4, max = 8) ),
    ...
    column(width=NCOL, numericInput("clin_detection_m", "Mean doubling rate for tumours", value = 6.5, min = 4, max = 8) ),
    column(width=NCOL, numericInput("clin_detection_sd", "Standard deviation of the doubling rate for tumours", value = 0.535) ),
    column(width=NCOL, sliderInput("log_norm_mean", "Tumour growth rate mean (lognormal) (\u03b1\u2081) [1/yr]", value = 1.07, min = 0.1, max = 1.399) ),
    column(width=NCOL, sliderInput("log_norm_sd", "Tumour growth rate SD (\u03b1\u2082) [1/yr]", value = 1.31, min = 0.1, max = 1.7) ),
   # Fixed
    column(width=NCOL, numericInput("max_size", "Maximum tumour size [mm]", value = 128) ),
   # Fixed
    column(width=NCOL, numericInput("start_size", "Tumour starting size (diameter) [mm]", value = 0.25) )
   # UNKNOWN: NA
  ),
  ...
  fluidRow(
    ...
    column(width=NCOL, sliderInput("beta_1", "Sensitivity of mammography (\u03b2\u2081) [mm] \u2014 how rapidly sensitivity changes with tumour size", value = 1.47, min = 0.1, max = 1.8) ),
    column(width=NCOL, sliderInput("beta_2", "Sensitivity of mammography (\u03b2\u2082) [mm] \u2014 size of tumour for which sensitivity is equal to 50%.", value = 6.51, min = 0.1, max = 8) ),
   # Fixed
   ...
   # Sen_VDG must be decreasing, and less than sensitivity_max
    ...
    column(width=NCOL, numericInput("Mammo_cdr", "Cancer detection rate [\u2030] high dense screens Mammo CEPAC", step = 0.1, value = 4.2, min = 1, max = 10) ),
    column(width=NCOL, sliderInput("MRI_cdr", "Cancer Detection Rate for MRI in Mammo negative women [\u2030]", step = 0.1, value = 5, min = 1, max = 10) ),
    column(width=NCOL, sliderInput("US_cdr", "Cancer Detection Rate for US in Mammo negative women [\u2030]", step = 0.1, value = 3, min = 1, max = 10) ),
   # UNKNOWN: NA
   # UNKNOWN: NA
  ...
  h2("Cost data"),
  fluidRow(
    column(width=NCOL, sliderInput("cost_strat", "Cost of stratification process [GBP]", value = 8.45, min = 0, max = 200) ),
    column(width=NCOL, sliderInput("costscreen", "Cost of screening [GBP]", value = 60.56, min = 0, max = 200) ),
    column(width=NCOL, sliderInput("cost_follow_up", "Cost of follow up [GBP]", value = 106, min = 0, max = 500) ),
    column(width=NCOL, sliderInput("cost_biop", "Cost of biopsy [GBP]", value = 290, min = 0, max = 1000) ),
    column(width=NCOL, numericInput("cost_DCIS", "Cost of treating ductal carcinoma in situ [GBP]", value = 9840, min = 0, max = 50000) ),
    column(width=NCOL, sliderInput("cost_US", "Cost of ultrasonography [GBP]", value = 52, min = 0, max = 200) ),
    column(width=NCOL, sliderInput("cost_MRI", "Cost of MRI [GBP]", value = 114, min = 0, max = 500) ),
...
cols = list(names = T, editableNames = F),
class = "numeric") ),
    column(width=NCOL, sliderInput("costvar", "Multiplier for Cost of Cancer treatment", value = 1, min = 0.1, max = 10) )
  ),
...
class = "numeric") ),
    ...
    column(width=NCOL, sliderInput("util_1to3", "Utility decrement (first yeara, set time independent) stages 1 to 3", step = 0.01, value = 0.82, min = 0, max = 1) ),
    column(width=NCOL, sliderInput("util_4", "Utility decrement (first year, set time independent) metastatic", step = 0.01, value = 0.75, min = 0, max = 1) ),
