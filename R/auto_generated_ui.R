# Code generated by parse_ui_table.R, Thu Jul 20 15:22:07 2023
# Edit input config in: data_raw/input_config.tsv:
# Edit parsing rules & layout in: dev/parse_ui_table.R
auto_generated_ui <- function(...) {
  ...
  tabsetPanel( ... ,
    ...
    tabPanel(title = "Strategy",
      ...
      sliderInput("start_age", "Age of an individual at start of simulation", step = 1, value = 38, min = 24, max = 60),
      ...
      sliderInput("discount_health", "Health discount rate", step = 0.001, value = 0.035, min = 0, max = 0.06),
      sliderInput("discount_cost", "Cost discount rate", step = 0.001, value = 0.035, min = 0, max = 0.06),
      sliderInput("screen_startage", "Screen start age", step = 1, value = 50, min = 40, max = 60),
      sliderInput("screen_endage", "Screen end age", step = 1, value = 70, min = 60, max = 80),
      uiOutput("strategy.widget")
    ),
    tabPanel(title = "Uptake",
      h3("NA"),
      sliderInput("uptakefirstscreen", "Probability that woman attends first screening", step = 0.01, value = 0.6, min = 0.4, max = 1),
      sliderInput("uptakeotherscreen", "Probability that woman attends further screening (having attended first)", step = 0.01, value = 0.85, min = 0.4, max = 1),
    ...
    tabPanel(title = "Cancer Growth",
      ...
      shinyMatrix::matrixInput("stage_by_size_mat", value = data.matrix(data.frame(v1=c(0.383,0.567,0.611,0.557,0,0), v2=c(0.033,0.111,0.180,0.208,0.723,0.563), v3=c(0.058,0.057,0.208,0.147,0.206,0.351), v5=c(0.525,0.265,0.120,0.088,0.071,0.086)),rownames.force=F),
                               ...
                               cols = list(names = T, editableNames = F),
                               class = "numeric"),
      numericInput("screen_detection_m", "Mean doubling rate for tumours at screen detection", step = 0.1, value = 6.1, min = 4, max = 8),
      numericInput("screen_detection_sd", "Standard deviation of the doubling rate for tumours at screen detection", value = 0.96),
      numericInput("clin_detection_m", "Mean doubling rate for tumours", step = 0.1, value = 6.5, min = 4, max = 8),
      numericInput("clin_detection_sd", "Standard deviation of the doubling rate for tumours", value = 0.535),
      sliderInput("log_norm_mean", "Tumour growth rate mean (lognormal) (\u03b1\u2081) [1/yr]", step = 0.01, value = 1.07, min = 0.01, max = 2),
      sliderInput("log_norm_sd", "Tumour growth rate SD (\u03b1\u2082) [1/yr]", step = 0.01, value = 1.31, min = 0.01, max = 2),
      # Fixed
      ...
      shinyMatrix::matrixInput("ca_size_cut", value = data.matrix(c(0.025, 5, 10, 15, 20, 30, 128),rownames.force=F),
                               rows = list(names = F, editableNames = F),
                               cols = list(names = T, editableNames = F),
                               class = "numeric"),
      uiOutput("cancer.growth.widget")
    ),
    tabPanel(title = "Screening",
      h3("Screening Performance"),
      sliderInput("prop_screen_detected", "Proportion of cancers in screening age range detected by screen", step = 0.01, value = 0.43, min = 0.3, max = 0.6),
      sliderInput("beta_1", "Sensitivity of mammography (\u03b2\u2081) [mm] \u2014 how rapidly sensitivity changes with tumour size", step = 0.01, value = 1.47, min = 0.1, max = 1.8),
      sliderInput("beta_2", "Sensitivity of mammography (\u03b2\u2082) [mm] \u2014 size of tumour for which sensitivity is equal to 50%.", step = 0.05, value = 6.5, min = 1, max = 20),
      # Fixed
      ...
      shinyMatrix::matrixInput("risk_cutoffs_procas", value = data.matrix(c(1.5,3.5,5,8,100),rownames.force=F),
                               rows = list(names = F, editableNames = F),
                               cols = list(names = T, editableNames = F),
                               class = "numeric"),
      shinyMatrix::matrixInput("risk_cutoffs_tert", value = data.matrix(c(1.946527,2.942792),rownames.force=F),
                               rows = list(names = F, editableNames = F),
                               cols = list(names = T, editableNames = F),
                               class = "numeric"),
      ...
      uiOutput("screening.widget")
    ),
    tabPanel(title = "Overdiagnosis",
      h3("False Positive and Overdiagnosis parameters"),
      numericInput("recall_rate", "Approx. UK recall rate", step = 0.01, value = 0.05, min = 0, max = 0.25),
      numericInput("biopsy_rate", "False-positive biopsy proportion", step = 0.01, value = 0.02, min = 0, max = 0.25),
      uiOutput("overdiagnosis.widget")
    ),
    tabPanel(title = "Cost",
      h3("Cost data"),
    ...
    tabPanel(title = "Utility",
      ...
      sliderInput("util_1to3", "Utility decrement (first year, set time independent) stages 1 to 3", step = 0.01, value = 0.82, min = 0, max = 1),
      sliderInput("util_4", "Utility decrement (first year, set time independent) metastatic", step = 0.01, value = 0.75, min = 0, max = 1),
      # Now function of util_1to3 and util_4, to replace them in advanced mode?
