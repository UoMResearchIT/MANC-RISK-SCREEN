...
screening_result <- function(Ca_size,VDG,MRI_screening,US_screening){
  ...
  Sensitivity <- if(
    exp((Ca_size - beta_2)/beta_1)/(1+exp((Ca_size-beta_2)/beta_1))>sensitivity_max){sensitivity_max}
  else{exp((Ca_size - beta_2)/beta_1)/(1+exp((Ca_size-beta_2)/beta_1))} #use to set max sensitivity 0.95
