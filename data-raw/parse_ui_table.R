...
#'
#' Writes a file "dev/auto_generated_ui.R" with a fluidPage object,
#' to be sourced into "app_ui.R". The format and parsing is controlled by
#' `write_chunk` below.
#'
...
parse_line <- function(line) {
  ...
  if (!line$valid) {
    msg <- str_flatten_comma(line[!is.empty(line)])
  ...
  line$type <- str_to_lower(glue::trim(line$type))
...
parse_numeric <- function(line) {
  ...
  args <- glue(str_flatten_comma(args))
